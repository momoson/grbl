...
#ifdef ACTIVE_A_AXIS
  uint8_t homing_axis_lock_a;    // Locks axes when limits engage. Used as an axis motion mask in the stepper ISR.
#endif
...
extern int32_t sys_position[N_AXIS_TOTAL];      // Real-time machine (aka home) position vector in steps.
extern int32_t sys_probe_position[N_AXIS_TOTAL]; // Last probe position in machine coordinates and steps.
